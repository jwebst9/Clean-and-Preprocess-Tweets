...
fent_count <- fent_words %>% anti_join(stop_words) %>% count(word, sort=T)

wordcloud(words=fent_count$word, freq=fent_count$n, min.freq = 1, max.words=100, 
          random.order=FALSE, rot.per=0.35, colors=brewer.pal(8, "Dark2"))

#suboxone
sub_words <- sub_tweets %>% select(text) %>% unnest_tokens(word, text)
sub_count <- sub_words %>% anti_join(stop_words) %>% count(word, sort=T)

wordcloud(words=sub_count$word, freq=sub_count$n, min.freq = 1, max.words=100, 
          random.order=FALSE, rot.per=0.35, colors=brewer.pal(8, "Dark2"))

#methadone
meth_words <- methadone_tweets %>% select(text) %>% unnest_tokens(word, text)
meth_count <- meth_words %>% anti_join(stop_words) %>% count(word, sort=T)

wordcloud(words=meth_count$word, freq=meth_count$n, min.freq = 1, max.words=100, 
          random.order=FALSE, rot.per=0.35, colors=brewer.pal(8, "Dark2"))
```
